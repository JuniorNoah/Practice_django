...
class MainViewSet(viewsets.ModelViewSet) :
    ...
    def list(self, request) :
        ...
        serializer = MainSerializer(self.get_queryset(), many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    
    # it's related to 'GET' / read(search) row from DB table
    def retrieve(self, request, uuid=None) :
        try :
            objects = MainPage.objects.get(id=uuid)
            serializer = MainSerializer(objects)
            return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def update(self, request, uuid=None) :
        objects = MainPage.objects.get(id=uuid)
        serializer = MainSerializer(objects, data=request.data)
        if serializer.is_valid(raise_exception=False) :
            serializer.save()
