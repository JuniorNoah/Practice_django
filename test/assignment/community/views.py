...
class HeadDetailView(APIView) :
    def get(self, request, head_id) :
        model = HeadModel.objects.filter(id=user_id)
        serializer = HeadSerializer(model, many=True)
        return Response(serializer.data)
...
class PostDetailView(APIView) :
    def get(self, request, post_id) :
        model = PostModel.objects.filter(id=user_id)
        serializer = PostSerializer(model, many=True)
        return Response(serializer.data)
...
class CommentDetailView(APIView) :
    def get(self, request, comment_id) :
        model = CommentModel.objects.filter(id=user_id)
        serializer = HeadSerializer(model, many=True)
        return Response(serializer.data)
...
class CommunityListView(APIView) :
    def get(self, request) :
        print('Add the community number you want at the end of urls to access the community')
        print('Or, type new community informations')
    ...
    def post(self, request) :
        ...
        serializer = CommunitySerializer(data=request.data, particial=True, community_admin_id=admin)
        if serializer.is_valid() :
            serializer.save()
            return Response(serializer.data, status=201)
        return Response(serializer.errors, status=400)
